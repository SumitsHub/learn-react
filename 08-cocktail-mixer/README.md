...
```js
<Outlet context={{ name: "John", age: 12 }} />
```
...
- action: route props and redirect
...
- example action, for more info refer to Newsletter page
...
- Hooks: useNavigation, useNavigate, useOutletContext, useLoaderData, useRouteError

```js
const navigation = useNavigation();
const isPageLoading = navigation.state === "loading";
// below -> when Form is used
...
const { id } = useLoaderData(); // loader of the current route component returning this
...
- useQuery, and queryClient.ensureQueryData
...
- good example of tagged templates
- present in assets/wrappers
- eample -
...
- exporting multiple default exported components from single file
...
#### About SPA (Single-Page Application)
...
React Router is a JavaScript library used in React applications to handle routing and navigation. It provides a <b>declarative</b> way to define the routes of an application and render different components based on the current URL. React Router allows developers to create a seamless, <b>client-side navigation</b> experience within a SPA by mapping URLs to specific components and managing the history and URL changes.
...
// Note - This calls after initial render, we need something to call while loading
useEffect(() => {
  fetchSomething();
}, []);
```
...
- bubbles up -> errorElement of parent route will be used if child don't have one
- no return from loader -> if loader of route don't return anything, meaning returns undefined
- wrong url -> navigating to route which is not present
...
#### SinglePageError Component - useRouteError() hook
...
#### useOutletContext() hook -> used to access value passed to <Outlet /> context
...
#### Global Loading and Context - IMP

- Global Loading -> useNavigation() hook
...
- loader has access to route paramater as prop named 'params'
...
#### Additional Check - Navigate from 'react-router-dom'
...
Newsletter.jsx -> replacing html 'form' with router 'Form'
...
App.jsx -> React router action in action
...
#### Try/Catch -> redirect from 'react-router-dom', an alternative to Navigate?
...
#### Submit State - navigation.state === 'submitting'
...
#### Query Params -> extracting query params in loader of route element
...
#### Redirects - fix for Netlify React Router issue
